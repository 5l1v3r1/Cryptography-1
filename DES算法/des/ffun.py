...
def S_replace(text):
    ...
    for k in range(8):
        ...
        for i in range(4):
            Sresult[4*k + i] = util.int2bit(tmp,4)[i]
    return Sresult
...
def ffun(text,k):
    extest = Extend(text)
    xortest = Xor(extest, k)
    srepstr = S_replace(xortest)
    prepstr = P_replace(srepstr)
    return prepstr
    # return P_replace(srepstr)


if __name__ == "__main__":
    keystr = '10101010101010101010111000101010'
    kstr = '101010101010101010101010111110000100101010101010'
    keylist = list(keystr)
    klist = list(kstr)
    print("".join(ffun(keylist,klist)))
